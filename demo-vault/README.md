...
- **Otherwise**, download `external-rename-handler-demo-vault.zip` from the [plugin releases](https://github.com/mnaoumov/obsidian-external-rename-handler/releases), unzip it, and in Obsidian choose `Manage vaults...` → `Open folder as vault` → select the `external-rename-handler-demo-vault-<version>` folder it unzips into.
